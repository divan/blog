...
TL;DR: a weekend project for transferring data via animated QR codes, written in Go. The Go code is reused for mobile apps using Gomobile, and in web application for automating testing QR codes parameters, built with GopherJS and Vecty framework. 
...
Quick research of prior work showed a [few](https://github.com/leonjza/qrxfer) [similar](https://volumeintegration.com/jumping-the-gap-data-transmission-over-an-air-gap/) [projects](http://stephendnicholas.com/posts/quicker-video-qr-codes), mostly as hackaton projects or even as a [bachelor's thesis](http://www.theseus.fi/bitstream/handle/10024/96359/Grasbeck_Max.pdf?sequence=1&isAllowed=y) and written in Java, Python or JavaScript. That means the code is not cross-platform and virtually non-reusable and has to be reimplemented from scratch. Luckily, due to huge popularity of QR codes, there is no shortage of libraries to work with them, and QR decoding is even embedded in the camera software of major smartphones manufacturers. (That's actually was one of the reasons of not exploring other ways to increase capacity of QR code, like colored QR codes, where color encodes additional layers, or even cooler stuff with luminance encoding like Apple uses for their [particle cloud in Apple Watch pairing process](https://www.youtube.com/watch?v=-WK4jiwlE5k)).

# TXQR

So that's how my weekend project started. Name TxQR stands for Transfer (Tx) via QR  codes.

The basic design is following. One client chooses the data to be sent, generates animated QR code and shows it in the loop, until the reader receives all the frames. Encoding is designed in such a way, that it allows for any particular order of frames, as well as dynamic changes in FPS or encoded chunks size. That was made for the case where reader is much slower and it can display a message "please decrease FPS on sender", and continue receiving the same file, even if the frame size has changed.

The protocol is quite simple – each frame starts with a prefix "NUM/TOTAL|", (where NUM and TOTAL are integer values for current and total frames respectively) and the rest is the file content. For binary content, original data is encoded using Base64 so only alphanumeric data is actually encoded in QR. Then frames are shown with a given FPS in the inifinite loop.

![TXQR protocol](/images/txqr_protocol.png)

It's that simple, and [here](https://github.com/divan/txqr) is a Go implementation of the protocol, along with a handy wrappers for encoding and decoding QR codes. The cool part was to make a mobile app that can use this code.

## Gomobile

And it was an extremely easy task thanks to [gomobile](https://github.com/golang/mobile).

You just write a normal Go code, then run `gomobile bind ...` and in a few seconds get prepared `.framework` or `.aar` file to be included into your iOS or Android project. In the project you reference to it as to any normal library, and get autocomplete and type information automatically.

I quickly built a simple iOS QR scanner in Swift (thanks to the [fantastic intro](https://medium.com/appcoda-tutorials/how-to-build-qr-code-scanner-app-in-swift-b5532406dd6b) on that by Simon Ng) and modified it to read animated QR codes, feed the decoded chunks into txqr decoder and display received file in a preview window.
...
I also wrote terminal tool that displays QR codes in the console for quickly testing the app. Combined, that worked suprisingly well - I was able to send small image in approximately ten seconds, but once I started to test on bigger files and playing with different FPS, I realized that terminal QR implementation frame rate wasn't enough to test higher rates, and that trying it manually is a dead end.

# TXQR Tester

![TXQR tester](/images/txqr_tester.jpg)

If I want to find the optimal combination of FPS, QR frame size and QR recovery level I have to run at least 1000 tests, manually changing parameters and writing down results to the spreadsheets, while holding phone near the screen. No way. I obviously should be able to automate that!

That's where idea of txqr tester app came in. First, I decided to use [x/exp/shiny](x/exp/shiny) which is an UI framework for desktop apps in Go, but it seems to as experimental as it is abandoned. That's pity as I played with `shiny` a year ago and it was promising for low-level desktop apps. When I tried it today, it didn't even compile. Seems like there is no incentive to develop desktop UI frameworks anymore – everything is in the web.

But web programming, you know, is still in the very early stages – programming languages just started to be supported by browsers via WASM, but they're just making their first baby steps. There is, of course, JavaScript, but friends don't let friends write web apps in JavaScript, so I decided to use one recent discovery of mine - framework [Vecty](https://github.com/gopherjs/vecty), which allows you to write frontends in Go and automagically transpile it to JS via suprisingly mature project [GopherJS](https://github.com/gopherjs/gopherjs).
...
Just to tease you, here is how you write web app in Go these days:
...
The real magic happens on compilation stage, though. You just run `gopherjs build` and in less than a second, you got your compiled JS ready to be included in your page or server by your server. When I first run it, I expected to see bunch of errors, warnings, unreadable messages, but no - it's blazingly fast, silent and only reports compilation errors as oneliners. In the browsers, by the way, if panic is thrown, you see stacktraces with references to Go files (not the transpiled JS) and line numbers in it. Isn't it cool?

### Testing the TXQR encoder parameters

So, in a few hours, I got the web app that allows me to configure range of testing parameters:

- FPS (frame per second)
- QR frame size (how many bytes to put in each QR frame)
- QR Recovery level (Low, Medium, High and/or Highest)

and initiate testing sequence for the mobile app.

![TXQR tester app](/images/txqr_app.png)

Mobile app, of course, should be automated as well – it needs to understand when the next testing round starts, handle timeouts (sometimes phone camera just can't pick up all frames and never get result), sends results to the app, etc.
...
- mobile app reads this QR code, and sends the websocket message with confirmation
...
The full testing cycle runs around 4 hours (the heaviest part – generation of animated QR runs in the browser, and as it's still JS under the hood, it uses only one CPU core), and I had to make sure that screen doesn't turn itself off, or some other application window doesn't cover the testing app. I set up testing with following parameters:
...
The best result was 1.4 secs, which is almost 9KB/s! This result has been recorded at rate of 11 frames per second and chunk size of 850 bytes with Medium recovery level.  In fact, however, at such encoding ratio and fps the probability of phone camera missing a frame was quite high, so in many cases it was just looping over and over, waiting for the missed frame until the test round timed out.

Here is the boxplots for chunk size and fps variables (note, timed out measurements recorded here as values of 30s):

#### Time vs Chunk size:

[![Time vs Size](/images/qr_size.png)](https://plot.ly/~divan0/3/)

As we can see above, there low chunk sizes result in too much overhead for QR encoding and overall transferring time skyrockets. There are some sane values like 550 and 900 bytes per chunk, but step left or right and we got timeouts due to missed frames again. 1000 bytes size is almost guaranteed miss of frames and time out.

#### Time vs FPS:

[![Time vs FPS](/images/qr_fps.png)](https://plot.ly/~divan0/2/)

FPS parameter, a bit to my surprise, didn't had that much effect. The best values seem to be 6-7 FPS, which equals to approximately 150ms between frames. The lower fps result in increasing wait time, and faster FPS yields missed frames.

#### Time vs QR Recovery Level:

[![Time vs Lvl](/images/qr_lvl.png)](https://plot.ly/~divan0/6/)

QR Recovery Level parameter showed clear connection between transferring time and the redundancy level. The clear winner here is Low level (7% redundancy), and there is no surprise – smaller redundancy of data results in more readable and smaller QR codes, which are easier to scan and faster to process. For data transfer purpose we probably don't need much redundancy at all. So safe values should be either Medium or Low.
...
This fun project proved to me that it's definitely possible to add unidrectional data transfer without any network connectivity at all using animated QR codes. While maximum data transfer rate was around 9KB/s, in vast majority of cases you can expect more modest rates – 1-2KB/s.
...
Last, but not least, I'm still amazed how productive you can be with Go, once you know what you're going to build – extra short edit-run loop time boosts experimentation, simple code and no class-hierarchies madness makes refactoring easy and nice chore, and cross-platform mindset allows you to reuse the same code in a server, in web app and in the mobile apps at the same time. There is also still a plenty of room for the optimization and speed ups, as I mostly did things in the most straightforward way.
